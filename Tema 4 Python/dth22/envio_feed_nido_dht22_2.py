...
dhtDeviceIn = adafruit_dht.DHT22(board.D4, use_pulseio=False)
dhtDeviceEx = adafruit_dht.DHT22(board.D17, use_pulseio=False)

while True:
    try:
        # Print the values to the serial port
        temperatureIn = dhtDeviceIn.temperature
        humidityIn = dhtDeviceIn.humidity
        print(
            "TempIn: {:.1f} C    HumidityIn: {}% ".format(
                temperatureIn, humidityIn
            )
        )
        temperatureEx = dhtDeviceEx.temperature
        humidityEx = dhtDeviceEx.humidity
        print(
            "TempEx: {:.1f} C    HumidityEx: {}% ".format(
                temperatureEx, humidityEx
            )
        )
...
#No se puede mandar más de 30 datos por minuto en nuestra cuenta de adafruit
    aio.send('tempinterior', temperatureIn)
    aio.send('huminterior', humidityIn)
    aio.send('tempexterior', temperatureEx)
    aio.send('humexterior',humidityEx)
