...
class Server:
    ...
    def run(self):
        while True:
            ...
            self.sock.sendto(response.SerializeToString(), address)
    ...
    def post_photo(self, request):
        ...
        if request.post_photo.photo_url in user.photos:
            ...
            response.error.message = 'Photo already exists'
            return response
        
        user.photos.append(request.post_photo.photo_url)
        response.success.message = 'Photo posted successfully'
        print(f'User {user.id} posted a new photo: {request.post_photo.photo_url}')
        return response
